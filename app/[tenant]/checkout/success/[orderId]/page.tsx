...
export default async function OrderSuccessPage({
  ...
  const isQuote = order.status === 'QUOTE_REQUESTED';
    ...
    <div dir="rtl" className="mx-auto max-w-2xl px-6 py-16">
      <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm">
        ...
        <h1 className="mt-4 text-2xl font-bold text-gray-900">
          {isQuote ? 'בקשתך התקבלה!' : 'ההזמנה התקבלה!'}
        </h1>
        <p className="mt-2 text-gray-600">
          תודה {order.customerName}. שלחנו אישור לכתובת {order.customerEmail}.
          <br />
          {isQuote
            ? `המשתלה תחזור אליך בטלפון ${order.customerPhone} עם הצעת מחיר מפורטת. אין חיוב בשלב זה.`
            : `המשתלה תיצור איתך קשר בטלפון ${order.customerPhone} לתיאום תשלום ואיסוף/משלוח.`}
        </p>
        <p className="mt-3 text-sm text-gray-400">
          {isQuote ? 'מספר פנייה' : 'מספר הזמנה'}: {order.id}
        </p>
